...
class PushtDataConfig(BaseDataConfig):
    video_keys = ["video.ego_view"]
    state_keys = ["state.end_effector"]
    action_keys = ["action.end_effector"]
    annotation_keys = ["annotation.human.action.task_description"]
    observation_indices = [0]
    action_indices = [0]
    ...
    def modality_config(self) -> dict[str, ModalityConfig]:
        modality_configs = {
            ...
            "annotation": ModalityConfig(delta_indices=self.observation_indices, modality_keys=self.annotation_keys),
        }
        return modality_configs
